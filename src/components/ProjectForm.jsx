...
const formSchema = Yup.object({
  ...
  enableGoals: Yup.bool()
    .required()
    .default(true),
  enableInsights: Yup.bool()
    .required()
    .default(true),
  enableReleases: Yup.bool()
    .required()
    .default(true),
  enableRaid: Yup.bool()
    .required()
...
export function projectToFormData(project) {
  ...
  // role map -> strings
  form.administrators = Object.keys(project.roles)
    .filter(v => project.roles[v] === Roles.administrator)
    .sort()
    .join("\n");
  form.authors = Object.keys(project.roles)
    .filter(v => project.roles[v] === Roles.author)
    .sort()
    .join("\n");
  form.members = Object.keys(project.roles)
    .filter(v => project.roles[v] === Roles.member)
    .sort()
    .join("\n");
...
export function formDataToProjectData(project, form) {
  ...
  // checkboxes -> list
  data.updateTypes = Object.values(UpdateTypes).filter(
    v => form[updateTypesToProps[v]]
  );

  // strings -> teams list
  data.teams = form.teams.trim()
    ? form.teams.split(splitLines).map(v => v.trim())
    : [];
  ...
  data.roles = fromPairs(
    [].concat(
      form.members
        ...
        .map(v => v.trim())
        .filter(v => !!v)
        .map(v => [v, Roles.member]),
      form.authors
        .split(splitLines)
        .map(v => v.trim())
        .filter(v => !!v)
        .map(v => [v, Roles.author]),
      form.administrators
        .split(splitLines)
        .map(v => v.trim())
        .filter(v => !!v)
        .map(v => [v, Roles.administrator]),
...
export default function ProjectForm({
  ...
  return (
    <Paper className={classes.paper}>
      ...
      <FormDescription title={title}>{description}</FormDescription>
      ...
      <Formik
        ...
        onSubmit={submitHandler({
          action: async data =>
            await action(formDataToProjectData(project, data)),
          knownErrors,
          setMessages
        })}
      >
            ...
            <Grid container spacing={2} direction="column">
              ...
              <Grid item xs={12} md={6}>
                ...
                <Typography variant="body2" paragraph>
                  To allow other users to access this project, enter their email
                  addresses, one per line. Administrators can modify project
                  settings. Authors can add and modify updates. Members can only
                  view updates. There is no need to add the same user to more
                  than one list.
                </Typography>
              </Grid>
              <Grid item xs={12} md={6}>
